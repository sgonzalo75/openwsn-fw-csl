...
This was done on 2014-10-04 20:15:06.673855.
*/
...
void neighbors_init(OpenMote* self) {
   ...
   if ( idmanager_getIsDAGroot(self)==TRUE) {
      (self->neighbors_vars).myDAGrank=0;
   } else {
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if ((self->neighbors_vars).neighbors[i].used==TRUE) {
         ...
         if ((self->neighbors_vars).neighbors[i].numTxACK==0) {
            ...
            rankIncrease = (uint16_t)((((float)(self->neighbors_vars).neighbors[i].numTx)/((float)(self->neighbors_vars).neighbors[i].numTxACK))*2*MINHOPRANKINCREASE);
         }
...
void debugNetPrint_neighbors(OpenMote* self, netDebugNeigborEntry_t* out){
   uint8_t idxIn;
   uint8_t idxOut;
   
   idxOut=0;
   for (idxIn=0;idxIn<MAXNUMNEIGHBORS;idxIn++) {
      if((self->neighbors_vars).neighbors[idxIn].used) {
         out[idxOut].last_addr_byte = (self->neighbors_vars).neighbors[idxIn].addr_64b.addr_64b[7];//last byte of the address; poipoi could be [0]; endianness
         out[idxOut].rssi = (self->neighbors_vars).neighbors[idxIn].rssi;
         out[idxOut].parentPreference = (self->neighbors_vars).neighbors[idxIn].parentPreference;
         out[idxOut].DAGrank = (self->neighbors_vars).neighbors[idxIn].DAGrank;
         memcpy(
            &out[idxOut].asn,
            &(self->neighbors_vars).neighbors[idxIn].asn.bytes0and1,
            sizeof((self->neighbors_vars).neighbors[idxIn].asn.bytes0and1)
         );
         idxOut++;
      }
   }  
