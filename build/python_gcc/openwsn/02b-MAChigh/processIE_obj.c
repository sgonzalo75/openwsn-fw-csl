...
This was done on 2014-10-04 20:15:12.682748.
*/
...
port_INLINE void processIE_prependMLMEIE(OpenMote* self, 
   ...
   // reserve space
 packetfunctions_reserveHeaderSize(self, 
      pkt, 
...
port_INLINE uint8_t processIE_prependSyncIE(OpenMote* self, OpenQueueEntry_t* pkt){
...
port_INLINE uint8_t processIE_prependSlotframeLinkIE(OpenMote* self, OpenQueueEntry_t* pkt){
   mlme_IE_ht mlme_subHeader;
   uint8_t    len;
   uint8_t    linkOption;
   uint16_t   slot;
 ...
 packetfunctions_reserveHeaderSize(self, pkt,5);
...
port_INLINE uint8_t processIE_prependOpcodeIE(OpenMote* self, 
      OpenQueueEntry_t* pkt,
 ...
 packetfunctions_reserveHeaderSize(self, pkt,sizeof(uint8_t));
   ...
   *((uint8_t*)(pkt->payload)) = uResCommandID;
   ...
   // reserve space
 packetfunctions_reserveHeaderSize(self, pkt, sizeof(mlme_IE_ht));
...
port_INLINE uint8_t processIE_prependBandwidthIE(OpenMote* self, 
      OpenQueueEntry_t* pkt, 
      uint8_t           numOfLinks, 
      uint8_t           slotframeID
   ){
...
port_INLINE uint8_t processIE_prependSheduleIE(OpenMote* self, 
      OpenQueueEntry_t* pkt,
      uint8_t           type,
      uint8_t           frameID,
      uint8_t           flag,
   ...
   for(i=0;i<SCHEDULEIEMAXNUMCELLS;i++) {
      if(cellList[i].linkoptions != CELLTYPE_OFF){
         ...
         // - [1B] link_type
 packetfunctions_reserveHeaderSize(self, pkt,5); 
 packetfunctions_htons(self, cellList[i].tsNum,    &(pkt->payload[0])); 
 packetfunctions_htons(self, cellList[i].choffset, &(pkt->payload[2]));
...
port_INLINE void processIE_retrieveSlotframeLinkIE(OpenMote* self, 
      OpenQueueEntry_t* pkt,
      uint8_t*          ptr
   ){
...
port_INLINE void processIE_retrieveOpcodeIE(OpenMote* self, 
      OpenQueueEntry_t* pkt,
      uint8_t*          ptr,
...
port_INLINE void processIE_retrieveBandwidthIE(OpenMote* self, 
      OpenQueueEntry_t* pkt,
      uint8_t *         ptr,
...
port_INLINE void processIE_retrieveSheduleIE(OpenMote* self, 
      OpenQueueEntry_t* pkt,
      uint8_t*          ptr,
