...
This was done on 2014-10-04 20:14:28.441043.
*/
...
void processIE_prependMLMEIE(OpenMote* self, 
...
uint8_t processIE_prependSyncIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt
);
uint8_t processIE_prependSlotframeLinkIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt
);
uint8_t processIE_prependOpcodeIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t              uResCommandID
);
uint8_t processIE_prependBandwidthIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t              numOfLinks, 
   uint8_t              slotframeID
);
uint8_t processIE_prependSheduleIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t              type,
   uint8_t              frameID,
   uint8_t              flag,
...
void processIE_retrieveSlotframeLinkIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t * ptr
); 
void processIE_retrieveOpcodeIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t*             ptr,
   opcode_IE_ht*        opcodeIE
); 
void processIE_retrieveBandwidthIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t *            ptr,
   bandwidth_IE_ht*     bandwidthIE
); 
void processIE_retrieveSheduleIE(OpenMote* self, 
   OpenQueueEntry_t*    pkt,
   uint8_t *            ptr,
