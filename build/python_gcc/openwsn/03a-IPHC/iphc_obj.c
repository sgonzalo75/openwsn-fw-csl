...
This was done on 2014-10-04 20:15:28.676420.
*/
...
void iphc_retrieveIPv6Header(OpenMote* self, OpenQueueEntry_t* msg, ipv6_header_iht* ipv6_header);
...
void iphc_retrieveIPv6HopByHopHeader(OpenMote* self, 
   OpenQueueEntry_t*    msg,
...
owerror_t iphc_sendFromForwarding(OpenMote* self, 
 ...
 packetfunctions_ip128bToMac64b(self, &(msg->l3_sourceAdd),&temp_src_prefix,&temp_src_mac64b);
   ...
   //dest and src on same prefix
      if ( neighbors_isStableNeighbor(self, &(msg->l3_destinationAdd))) {
 ...
 openserial_printCritical(self, COMPONENT_IPHC,ERR_INVALID_FWDMODE,
                            (errorparameter_t)0,
                            (errorparameter_t)0);
        } 
        dam = IPHC_DAM_ELIDED;
 ...
 iphc_prependIPv6HopByHopHeader(self, msg, msg->l4_protocol, nh, rpl_option);
   ...
   if(ipv6_header->next_header!=IANA_IPv6ROUTE  && packetfunctions_isBroadcastMulticast(self, &(msg->l3_destinationAdd))==FALSE)   {
   ...
   if ( iphc_prependIPv6Header(self, msg,
            tf,
...
owerror_t iphc_sendFromBridge(OpenMote* self, OpenQueueEntry_t *msg) {
   msg->owner = COMPONENT_IPHC;
   // error checking
   if ( idmanager_getIsBridge(self)==FALSE) {
 openserial_printCritical(self, COMPONENT_IPHC,ERR_BRIDGE_MISMATCH,
                            (errorparameter_t)1,
                            (errorparameter_t)0);
      return E_FAIL;
...
void iphc_receive(OpenMote* self, OpenQueueEntry_t* msg) {
   ...
   // then regular header
 iphc_retrieveIPv6Header(self, msg,&ipv6_header);
   
   if ( idmanager_getIsBridge(self)==FALSE ||
 packetfunctions_isBroadcastMulticast(self, &(ipv6_header.dest))) {
 packetfunctions_tossHeader(self, msg,ipv6_header.header_length);
      
      if (ipv6_header.next_header==IANA_IPv6HOPOPT) {
         
         // retrieve hop-by-hop header (includes RPL option)
 iphc_retrieveIPv6HopByHopHeader(self, 
            msg,
            &ipv6_hop_header,
            &rpl_option
         );
...
void iphc_retrieveIPv6Header(OpenMote* self, OpenQueueEntry_t* msg, ipv6_header_iht* ipv6_header) {
 ...
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),&(msg->l2_nextORpreviousHop),&ipv6_header->src);
         break;
      case IPHC_SAM_16B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length)),ADDR_16B,&temp_addr_16b,OW_BIG_ENDIAN);
 ...
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),&temp_addr_64b,&ipv6_header->src);
         break;
      case IPHC_SAM_64B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length)),ADDR_64B,&temp_addr_64b,OW_BIG_ENDIAN);
         ipv6_header->header_length += 8*sizeof(uint8_t);
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),&temp_addr_64b,&ipv6_header->src);
         break;
      case IPHC_SAM_128B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length)),ADDR_128B,&ipv6_header->src,OW_BIG_ENDIAN);
 ...
 packetfunctions_mac64bToIp128b(self,  idmanager_getMyID(self, ADDR_PREFIX),&temp_addr_64b,&ipv6_header->dest);
         break;
      case IPHC_DAM_128B:
 packetfunctions_readAddress(self, ((uint8_t*)(msg->payload+ipv6_header->header_length)),ADDR_128B,&ipv6_header->dest,OW_BIG_ENDIAN);
         ipv6_header->header_length += 16*sizeof(uint8_t);
         break;
      default:
 openserial_printError(self, 
...
#ifndef FLOW_LABEL_RPL_DOMAIN
   // RPL option
 packetfunctions_reserveHeaderSize(self, msg,sizeof(rpl_option_ht));
