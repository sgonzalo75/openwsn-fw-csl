...
This was done on 2014-10-04 20:15:36.073493.
*/
...
void forwarding_createFlowLabel(OpenMote* self, uint32_t* flow_label,uint8_t flags);
#endif
//=========================== public ==========================================
...
#ifdef FLOW_LABEL_RPL_DOMAIN
 forwarding_createFlowLabel(self, &flow_label,0x00);
#endif
...
void forwarding_sendDone(OpenMote* self, OpenQueueEntry_t* msg, owerror_t error) {
 ...
 openqueue_freePacketBuffer(self, msg);
      ...
      switch(msg->l4_protocol) {
         case IANA_TCP:
 opentcp_sendDone(self, msg,error);
            break;
         case IANA_UDP:
...
void forwarding_receive(OpenMote* self, 
   ...
   if (
         (
 idmanager_isMyAddress(self, &ipv6_header->dest)
            ||
 ...
 forwarding_createRplOption(self, rpl_option, rpl_option->flags);
         ...
         // forwarding_createFlowLabel(self, &(ipv6_header->flow_label),flags);
         #endif
 ...
 openqueue_freePacketBuffer(self, msg);
         ...
         if ( forwarding_send_internal_SourceRouting(self, msg, ipv6_header)==E_FAIL) {
...
void forwarding_getNextHop(OpenMote* self, open_addr_t* destination128b, open_addr_t* addressToWrite64b) {
   ...
   } else if ( neighbors_isStableNeighbor(self, destination128b)) {
      // IP destination is 1-hop neighbor, send directly
 packetfunctions_ip128bToMac64b(self, destination128b,&temp_prefix64btoWrite,addressToWrite64b);
   } else {
      // destination is remote, send to preferred parent
 neighbors_getPreferredParentEui64(self, addressToWrite64b);
   }
}
...
void forwarding_createFlowLabel(OpenMote* self, uint32_t* flow_label,uint8_t flags){
     ...
     instanceId= icmpv6rpl_getRPLIntanceID(self);
     rank= neighbors_getMyDAGrank(self);
