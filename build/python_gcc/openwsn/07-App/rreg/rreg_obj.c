...
This was done on 2014-10-04 20:16:10.595134.
*/
/**
...
owerror_t rreg_receive(OpenMote* self, OpenQueueEntry_t* msg,
                     coap_header_iht*  coap_header,
                     coap_option_iht*  coap_options);
void rreg_timer(OpenMote* self);
void rreg_sendDone(OpenMote* self, OpenQueueEntry_t* msg,
                      owerror_t error);
uint8_t hexToAscii(OpenMote* self, uint8_t hex);

//=========================== public ==========================================

void rreg_init(OpenMote* self) {
 ...
 opencoap_register(self, &rreg_vars.desc);
   
   // start a timer to register to the CoAP Resource Directory every RREGPERIOD
   rreg_vars.timerId    = opentimers_start(self, 
      RREGPERIOD,
      TIMER_PERIODIC,
      TIME_MS,
      rreg_timer
...
*/
owerror_t rreg_receive(OpenMote* self, 
 ...
 scheduler_push_task(self, rreg_timer,TASKPRIO_COAP);
...
void rreg_timer(OpenMote* self) {
   ...
   pkt = openqueue_getFreePacketBuffer(self, COMPONENT_RREG);
   if (pkt==NULL) {
 ...
 openqueue_freePacketBuffer(self, pkt);
   ...
   // write CoAP payload
 opencoap_writeLinks(self, pkt);
   
   numOptions = 0;
   
   // URI-query
 packetfunctions_reserveHeaderSize(self, pkt,sizeof(rreg_uriquery)-1+2);
   memcpy(&pkt->payload[0],&rreg_uriquery,sizeof(rreg_uriquery)-1);
   temp8b = idmanager_getMyID(self, ADDR_16B)->addr_16b[1];
   pkt->payload[sizeof(rreg_uriquery)-1] = hexToAscii(self, (temp8b>>4) & 0x0f);
   pkt->payload[sizeof(rreg_uriquery)-0] = hexToAscii(self, (temp8b>>0) & 0x0f);
...
*/
void rreg_sendDone(OpenMote* self, OpenQueueEntry_t* msg, owerror_t error) {
...
*/
port_INLINE uint8_t hexToAscii(OpenMote* self, uint8_t hex) {
   if (hex<0x0a) {
      return '0'+(hex-0x00);
   } else {
      return 'a'+(hex-0x0a);
